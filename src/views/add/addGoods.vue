<!--
 * @Date: 2022-06-11 17:05:01
 * @LastEditTime: 2022-06-30 16:00:27
-->
<template>
			...
			>
				...
				:class="showAddCategory ? 'showEdit' : ''">
					...
					<el-form-item label="商品种类" prop="name">
						<el-input v-model="categoryForm.name"></el-input> </el-input>
					</el-form-item>
          <el-form-item label="种类描述" prop="description">
						<el-input v-model="categoryForm.description"></el-input> </el-input>
					</el-form-item>
          <el-form-item>
            <el-button
              type="primary"
...
export default {
  ...
  computed:{
	...
	created(){
		if(this.$route.query.resturant_id){
			this.restaurant_id = this.$route.query.resturant_id;
		}else{
			this.restaurant_id = Math.ceil(Math.random() * 10);
			this.$msgbox({
				title:'提示',
				message:'添加商品需要选择一个商铺，先去选择商铺吗？',
	...
	.form {
		...
		&:hover {
			box-shadow: 0 0 8px 0 rgba(232, 237, 250, 0.6),
				0 2px 4 px 0 rgba(232, 237, 250, 0.5);
			border-radius: 6px;
